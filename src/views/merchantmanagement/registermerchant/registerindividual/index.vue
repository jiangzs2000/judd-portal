  ...
  <div class="app-container">
    <el-form ref="requestForm" :model="requestForm" :rules="requestRules" label-width="20%" size="mini" class="requestForm">
      ...
      <div>
        <p>支付设置</p>
        <div class="fields-block">
          <el-form-item label="支付通道费：" prop="serviceChargePercent">
...
import { registerIndividual, loadFRARequest } from '@/api/merchant'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      requestRules: {
        ...
        paySalesAgreement: {
          assumePerson: [{ required: true, trigger: ['blur', 'change'], message: '请选择手续费承担方' }]
        }
      },
      checkAll: false,
  ...
  mounted() {
    getBanksByMerchantType(2).then(res => {
      if (res.code === '0000') {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs.requestForm.validate(valid => {
        if (valid) {
          this.loading = true
          registerIndividual(this.requestForm).then(res => {
            if (res.code === '0000') {
              // this.$router.push({ name: 'RegisterIndividualResult' })
              this.requestForm = res.data
              this.$message({
                message: res.msg,
                type: 'success'
