    ...
    <div>
      <div>
        <ul>
          ...
          <li><span class="field-name">退款日期： </span><span class="field-value">{{ orderDetail==null?'':orderDetail.date }}</span></li>
          <li><span class="field-name">原平台订单号：</span><span class="field-value">{{ orderDetail==null?'':orderDetail.oriOrderNo }}</span></li>
          <li><span class="field-name">原交易日期： </span><span class="field-value">{{ orderDetail==null?'':orderDetail.oriDate }}</span></li>
          ...
          <li><span class="field-name">退款金额（元）： </span><span class="field-value">{{ orderDetail==null?'':orderDetail.amount | F2Y() }}</span></li>
          <li><span class="field-name">订单状态： </span><span class="field-value">{{ orderDetail==null?'':orderDetail.state }}</span></li>
          <li><span class="field-name">费用金额（元）： </span><span class="field-value">{{ orderDetail==null?'':orderDetail.feeAmount | F2Y() }}</span></li>
          <li><span class="field-name">对账日期： </span><span class="field-value">{{ orderDetail==null?'':orderDetail.merCheckDate }}</span></li>
        </ul>
      </div>
...
<script>
import { getRefundOrderDetail } from '@/api/orders'
export default {
  ...
  created() {
    getRefundOrderDetail(this.orderNo).then(res => {
      if (res.code === '0000') {
