...
import { getFundInOrdersByQuery } from '@/api/orders'
...
export default {
  ...
  methods: {
    ...
    viewDetial(row) {
      // console.log('row', row)
      this.$router.push({ name: 'FundInOrderDetail', params: { orderNo: row.orderNo }})
    },
    closeEventHandler(event) {
      // console.log('closeEventHandler', event)
      if (event.name === 'FundInOrderList') {
        console.log('FundInOrderList closed')
